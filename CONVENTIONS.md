...
- The generated security-policy bootstrap has a narrow Node compatibility boundary: `scripts/ci/generated/security-preflight.mjs` is compiled from canonical Civet using the pinned compiler for execution before candidate dependencies are installed. The runtime helper is not consumed by hosted workflows; its existence is not scan, approval, or ingestion evidence. Local preflight remains Bun/Civet. Never author behavior in the generated file; use `ci:preflight:generate` and require the non-mutating `ci:preflight:check` in static verification.
