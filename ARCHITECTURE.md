...
- **Core:** normal pre-push runs only the deterministic `verify:core` gate. Hosted CI retains all five lanes: core, coverage, integration, graph, and browser, with all four 100% coverage thresholds unchanged. Heavy CI runs on pull requests and explicit manual dispatch, not automatically on push.
- **Convention:** pre-push binds destination-scoped security preflight and the core gate to the exact executing and PATH-resolved Bun pin, actual destination, current clean HEAD, and pushed ref updates. A changed tree, historical receipt, different destination, or missing prerequisite cannot authorize a push. Environment-heavy lanes remain mandatory in hosted CI rather than sequential local push blockers.
- **Capability:** licensed private CodeQL, code-scanning ingestion, and dependency review require explicit activation plus a read-only capability probe. Unconfigured checks report not run; configured failures block. Public checks remain active and private Scorecard results are never published publicly.
- **Implementation:** typed Civet owns security policy. Local commands run it under Bun. GitHub's trusted preflight bootstrap uses a deterministic, checked-in Node-compatible compilation so it can inspect policy before installing or executing candidate workspace dependencies. `ci:preflight:check` proves exact generated bytes without repairing them; neither Biome nor pre-commit formatting owns that artifact. Hosted permissions and service availability are independently rechecked after push.
