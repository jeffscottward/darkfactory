...
- **Core:** every source push runs the same five lanes as hosted CI: core, coverage, integration, graph, and browser. Environment-heavy lanes are required locally, not deferred.
- **Convention:** pre-push binds those gates to the actual destination, current clean HEAD, and pushed ref updates. A changed tree, historical receipt, different destination, or missing local prerequisite cannot authorize a push.
