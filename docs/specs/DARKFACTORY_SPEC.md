...
`bun run verify` remains the complete sequential local lifecycle, and `bun run ci` invokes it. Husky pre-commit stays staged/focused. Pre-push binds Git's actual destination and every non-deletion pushed ref to current HEAD and requires clean source before hosted security capability preflight. After preflight passes, immutable `verify:core`, `verify:coverage`, `verify:integration`, `verify:graph`, and `verify:browser` run sequentially, stopping at the first failure. Success requires all five lanes to pass and a final clean-source and unchanged-HEAD check. Missing prerequisites, stale evidence, detected source mutation, or any failed preflight or lane blocks publication; remaining lanes do not run after failure. GitHub Actions repeats all five lanes concurrently after pinned Bun/Node/Corepack setup and frozen pnpm installation. Bun and Turbo still orchestrate those lanes; Vitest alone runs through the package-local `corepack pnpm exec` path under Node because Bun 1.3.14 misloads Vitest's Vite `zod` dependency and lacks the V8 `node:inspector` coverage APIs. Local success does not certify future GitHub service availability or token permissions.

CI validates every PR (including forks, nondefault targets, and docs-only changes), pushes only to the verified default branch, and explicit `workflow_dispatch` runs. A feature-branch push without a PR intentionally has no automatic CI; manual full validation is available. Generate the default-branch filter as a literal from the target's actual default branch and update it on rename, never interpolate it at runtime. Do not add path/actor filters, `pull_request_target`, or conditional lane omissions. Preserve read-only permissions, `persist-credentials: false`, fork secret isolation, and required approval boundaries.
...
- [ ] **DF-103 · Core · P1/W · depends: DF-101.** Husky pre-commit is staged/focused and calls scripts. Pre-push binds the actual destination and every non-deletion ref to clean, unchanged HEAD, checks hosted security capabilities, and runs all five immutable local lanes sequentially without bypasses or source/ref mutation. **Accept/evidence:** hook fixtures, destination/policy truth tables, each lane's failure propagation, and unchanged CI lane coverage. **Supersedes:** the former core-only pre-push scheduling rule under the user-directed full local CI parity requirement.
...
- [ ] **DF-105 · Core · P1/W · depends: DF-102.** CI uses frozen pnpm install, safe caches, isolated Postgres, migrations/seeds, config validation, and all deterministic gates in five concurrent lanes with `fail-fast: false`. Preserve the stable `Verification (core/coverage/integration/graph/browser)` contexts under the PR/default-branch-push/manual event contract in section 9, validation-only supersession cancellation, and isolated non-cancelling manual runs; never filter paths or serialize lanes. **Accept/evidence:** semantic event/concurrency/setup-pinning/grouped-update checks plus workflow and terminal result for every matrix lane on the latest eligible revision. **Supersedes:** S01.
...
- [ ] **DF-107 · Convention · P1/L · depends: DF-102–DF-106.** Every commit is focused and pushed only after all locally reproducible CI lanes and applicable hosted capability preflight accept the exact pushed source. Environment-heavy gates require prepared local prerequisites rather than deferral; hosted CI still repeats them in clean runners. Unconfigured licensed features are explicit not-run states, never passing analysis evidence. **Accept/evidence:** commit/evidence ledger, full pre-push lane transcript, and hosted terminal results. **Supersedes:** the former core-only local publication rule.
- [ ] **DF-108 · Convention · P6/L · depends: DF-102–DF-106.** Every eligible validation event is followed until all CI lanes are terminal; repository-owned failures are reproduced/fixed/rerun. A feature-branch push without a PR has no expected automatic CI; do not wait for nonexistent checks or label their absence green. **Accept/evidence:** CI URLs, exact SHA/attempt/conclusions, and repair loop notes. **Supersedes:** the original every-push trigger assumption.
