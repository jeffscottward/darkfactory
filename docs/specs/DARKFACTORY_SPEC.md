...
`bun run verify` remains the complete sequential local lifecycle, and `bun run ci` invokes it. Husky pre-commit stays staged/focused. Pre-push verifies the actual destination and pushed-source binding, performs hosted security capability preflight, then requires all five local lanes; missing environment prerequisites and stale generated evidence are blocking. GitHub Actions repeats all five lanes concurrently after pinned Bun/Node/Corepack setup and frozen pnpm installation. Bun and Turbo still orchestrate those lanes; Vitest alone runs through the package-local `corepack pnpm exec` path under Node because Bun 1.3.14 misloads Vitest's Vite `zod` dependency and lacks the V8 `node:inspector` coverage APIs. Local success does not certify future GitHub service availability or token permissions.
...
- [ ] **DF-103 · Core · P1/W · depends: DF-101.** Husky pre-commit is staged/focused and calls scripts. Pre-push binds the actual destination and non-deletion refs to clean tested source, checks applicable hosted security capabilities, and runs all five local lanes without bypasses. **Accept/evidence:** hook fixtures, destination/policy truth tables, each lane's failure propagation, and unchanged CI lane coverage. **Supersedes:** the former core-only pre-push scheduling rule under the user-directed full local CI parity requirement.
