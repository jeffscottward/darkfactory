...
15. Every pushed commit follows a focused green gate; follow all eligible PR/manual CI and independently triggered security checks to completion.
...
bun run verify:static       bun run verify:core:ci     bun run test:e2e-helpers
bun run ci
```
...
The stable required contexts are `Verification (core)`, `Verification (coverage)`, `Verification (integration)`, `Verification (graph)`, and `Verification (browser)`. All five remain concurrent with `fail-fast: false`, with no serial matrix limit. Validation-only concurrency uses workflow plus PR number or full branch ref so a new revision cancels obsolete work without coupling unrelated PRs/refs. Manual groups use a distinct `manual-<run_id>` suffix and never cancel in-progress work. Ordinary groups must not include SHA, run ID, or event type; deployment workflows must not inherit validation cancellation. A cancelled/superseded revision is not passed: require the latest revision's complete successful checks. Inspect protections, effective rulesets, merge queues, deployment event dependencies, and fork approval policy before rollout; unresolved or incompatible event obligations stop migration.

The approved solo-maintainer governance in `AGENTS.md` sets `required_approving_review_count: 0` and `require_last_push_approval: false`, retaining documented exact-head technical review and actual verification. Preserve every effective required check identity/app, strict up-to-date checks, and all other protection; read back explicitly authorized changes before relying on them. This specification does not establish live settings. Independent approval is required only when configured by the effective rules and must qualify for the final head; unknown or incompatible rules block acceptance. Never invent approval, bypass as admin, grant access, or silently change policy. Security-bootstrap review covers the complete helper/generator/generated-artifact boundary and hosted consumers, not only the latest diff; generated bytes prove no scan, approval, or ingestion. Successor publication remains held until the public follow-up is accepted.

Merge acceptance requires the latest reviewed PR head's successful required checks and exact identity between its Git tree and the resulting merged tree. Record both SHAs and the shared tree; do not imply CI ran on the merge SHA or call intentional absence of a heavy main rerun a green, missing, or skipped check. Unexpected direct-main changes require explicit full manual validation at their exact SHA before acceptance.

Before actual deployment, explicitly dispatch full CI on a branch or tag resolving to the intended deployment SHA, verify the observed run's `head_sha` equals that SHA, and require all five lanes to succeed. Same-tree PR evidence is not deployment-SHA execution evidence. This prerequisite is operator policy, not an automated gate in the current deploy CLI; preserve deployment authorization, environment approval, secret isolation, runtime probes, and rollback evidence.

The graph CI lane builds current source, then runs canonical `graph:check` and semantic `graph:verify`. Committed source-fingerprint bookkeeping is not an acceptance gate for unrelated edits. Local stale-source detection and required refresh before graph use remain intact.

Pin reviewed action releases to immutable full 40-hex SHAs. Test setup semantics and pin validity rather than a particular release hash: another valid pin must work without a test rewrite. Preserve declared toolchain versions, Bun version-file input, pnpm setup ID/output consumption and `run_install: false`, required ordering, and frozen install. Dependabot remains GitHub Actions-only, weekly, with one wildcard `github-actions` group and `open-pull-requests-limit: 1`; keep all CodeQL `init`/`analyze`/`upload-sarif` updates atomic and compatible. This bounds ordinary version-update PRs, not security-update batching or existing proposals. Review upgrades; do not enable unverified package updater support.

Hosted security follows [the capability rollout contract](../capabilities-and-deployment.md#hosted-security-capabilities) and [preflight operations](../security-preflight.txt). Public active checks cannot opt out. Private CodeQL and Dependency Review require their own exact lowercase `true` selections plus positive GitHub feature API probes; private CodeQL also requires confirmed licensing. False/unset private selections are **NOT CONFIGURED / NOT RUN**, not passing scans. Configured-but-unavailable, authentication/network, invalid-value, and unknown-state failures block. `DF_CODE_SCANNING_UPLOAD_ENABLED` independently gates private SARIF ingestion; analysis, ingestion, and public publication are distinct. Free Scorecard analysis always runs, including privately and after capability failure without hiding that failure; private/unknown repositories never public-publish. Preserve protected CodeQL Actions/JavaScript and Dependency Review checks alongside the five verification contexts. CodeQL analyzes with `upload: never` before separate authorized ingestion; retain generated CodeQL and Scorecard SARIF artifacts for exactly seven days, and block on artifact upload failures. No `continue-on-error` or success wrappers.

Hosted PR preflight executes the generated Node helper from the exact trusted PR base SHA. If absent, first merge a helper-only bootstrap PR under unchanged baseline workflows/protections, then adopt successor workflows in a PR based on that merge. Never execute the PR's helper as a fallback, weaken required checks, or bypass protections to break this dependency.

The [one-shot prompt](DARKFACTORY_ONE_SHOT_PROMPT.md#github-actions) defines the matching generation header. Deterministic failures need fixes; a single verified transient at unchanged SHA permits rerunning only failed jobs and necessary dependencies, not successful independent lanes. Cancellation never hides a known failure, and missing checks or waiting approval are not green.
...
- [ ] **DF-103 · Core · P1/W · depends: DF-101.** Husky pre-commit is staged/focused and calls scripts. Pre-push binds the actual destination and every non-deletion ref to clean, unchanged HEAD, verifies executing/PATH Bun pins and hosted security capabilities, and runs only immutable `verify:core` without bypasses or source/ref mutation. Environment-heavy gates remain mandatory concurrent CI lanes. **Accept/evidence:** hook fixtures, destination/policy truth tables, core failure propagation, and unchanged hosted lane coverage. **Supersedes:** the five-sequential-local-lanes requirement.
...
- [ ] **DF-105 · Core · P1/W · depends: DF-102.** CI uses frozen pnpm install, safe caches, isolated Postgres, migrations/seeds, config validation, and all deterministic gates in five concurrent lanes with `fail-fast: false`. Preserve the stable `Verification (core/coverage/integration/graph/browser)` contexts under the PR/manual-only event contract in section 9, validation-only supersession cancellation, and isolated non-cancelling manual runs; never filter paths or serialize lanes. **Accept/evidence:** semantic event/concurrency/setup-pinning/grouped-update checks plus workflow and terminal result for every matrix lane on the latest eligible revision. **Supersedes:** S01 and automatic default-branch heavy reruns.
- [ ] **DF-106 · Core · P1/W · depends: DF-105.** CI checks format/lint/type/build/unit/contract, deterministic coverage generation and committed-artifact byte staleness, integration, OpenAPI/Graphify/docs, and E2E/a11y; browser evidence is retained only after its scanner succeeds. **Accept/evidence:** terminal matrix jobs, current coverage artifact check, browser scanner result, and upload-condition contract. **Supersedes:** none.
- [ ] **DF-107 · Convention · P1/L · depends: DF-102–DF-106.** Every commit is focused and pushed only after deterministic core and destination-scoped capability preflight accept the exact pushed source. Environment-heavy verification remains explicit and runs in clean CI rather than serializing local publication. Exact-head technical review and real required checks are mandatory before merge; independent approval follows effective rules. Unconfigured licensed features remain explicit not-run states, never passing analysis evidence. **Accept/evidence:** commit/evidence ledger, core/preflight transcript, full review record, and hosted terminal results. **Supersedes:** the all-environments-before-push scheduling rule.
- [ ] **DF-108 · Convention · P6/L · depends: DF-102–DF-106.** Every eligible PR/manual validation event is followed until all CI lanes are terminal; repository-owned failures are reproduced/fixed/rerun. Ordinary pushes, including default-branch merges, have no expected automatic heavy CI; do not wait for nonexistent runs or label absence green, missing, or skipped. Merge evidence pairs successful latest reviewed PR checks with exact merged-tree identity; independent security checks remain required under their own event policy. **Accept/evidence:** CI URLs, exact SHA/attempt/conclusions, merge/tree receipt, and repair loop notes. **Supersedes:** the original every-push and automatic post-merge trigger assumptions.
- [ ] **DF-109 · Core · P6/L · depends: DF-098, DF-105–DF-108.** Actual deployment requires protected authorization and successful full manual CI at the intended deployment SHA; untrusted PRs never receive deploy secrets. Dispatch on a branch/tag resolving to that SHA, verify the observed run's `head_sha`, and require all five lanes to succeed. This is current operator policy, not an implemented deployment-workflow/CLI gate. **Accept/evidence:** exact-SHA run/attempt/lane results plus target authorization, environment approval, secret boundary, runtime and rollback evidence; future workflow permissions/dependencies must preserve that boundary. **Supersedes:** S08.
...
At each integration boundary: run focused tests while iterating, run the phase gate, update generated artifacts/Graphify/docs, commit one coherent green change, push to the PR, follow its required checks to terminal success, then release the dependent owner. Do not add an automatic post-merge heavy run; retain the merge/tree receipt and follow independently triggered security checks.
