...
- `pre-push`: bind Git's actual destination and every non-deletion pushed ref to current HEAD; require clean source and executing/PATH-resolved Bun matching the exact `.bun-version` pin before destination-scoped security preflight. After preflight passes, run only immutable `verify:core`. Any prerequisite, preflight, core, or final clean-source/unchanged-HEAD failure blocks the push. Environment-heavy lanes remain explicit in full verification and mandatory hosted CI, not per-push hook stages. Never bypass the hook or relabel not-run work as passed.
...
Require documented exact-head technical review and real successful current required checks before merge. Independent GitHub approval is required only by effective repository/organization rules and must qualify for the final head. Preserve required check names/app identities, strict up-to-date checks, and other protections; unknown or incompatible rules block acceptance. Never invent approval, bypass as admin, grant access, or change settings merely to obtain a merge. Security-bootstrap review covers the entire helper/generator/generated-artifact trust boundary and hosted consumers. Successor publication remains held until the public follow-up is accepted.
