...
`develop → focused verification and artifact updates → focused commit → deterministic pre-push → five-lane PR CI → exact-head technical review and merge/tree verification → explicit exact-SHA full CI before authorized deployment`

Root `bun run` scripts and Turborepo tasks are the source of truth. `verify:static` owns static checks, lint, typecheck, builds, docs, and generated-artifact freshness. Local pre-push runs only `verify:core`: static plus unit, contract, and operations tests. Full `verify` and its `ci` alias instead compose `verify:core:ci` (static plus scoped `test:e2e-helpers`) with coverage, integration, graph, and browser lanes; unit/contract/operations run once under coverage, with all four 100% thresholds unchanged. GitHub Actions runs all five required lanes concurrently and builds fresh Graphify output before graph verification. Do not turn pre-push into five sequential local lanes. Pre-commit stays staged and focused. Every source push must bind Git's actual destination and every non-deletion ref to clean, unchanged current HEAD, require both executing and PATH-resolved Bun to match `.bun-version`, then pass destination-scoped security preflight before core. Failures or source mutation block the push. Environment-heavy verification remains explicit in full verification and mandatory hosted CI, not a per-push hook prerequisite. Never bypass a failing gate, disable a test, or skip hooks to make a change pass. A badge, prior run, generated file, or agent self-report is not evidence for the current revision; pending, skipped, cancelled, timed-out, blocked, or unobserved work is not green.
...
Before actual deployment, explicitly dispatch full CI on a branch or tag resolving to the intended deployment SHA, verify the observed run's `head_sha` equals that SHA, and require all five lanes to succeed. A same-tree PR receipt does not replace this deployment evidence. This is an operator acceptance policy, not an automated gate in the current deploy CLI; preserve authorization, environment approval, least privilege, runtime probes, and rollback evidence.

Private licensed security capabilities require explicit operator activation and positive capability evidence; an unconfigured scan must report **not configured / not run**, never “analysis passed.” Public security analysis remains enabled. A configured but unavailable capability is a blocking error, not a successful skip. Do not change billing or repository visibility to make CI pass. Local checks cannot guarantee hosted service availability or token permissions, so follow every triggered workflow to a terminal result after pushing. See [security preflight](docs/security-preflight.txt).
