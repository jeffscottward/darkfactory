...
Root `bun run` scripts and Turborepo tasks are the source of truth. `bun run ci` must mirror GitHub Actions. Do not use `pnpm run` for lifecycle commands or bare `pnpm ci` as a gate; pnpm is reserved for frozen installation, package/workspace resolution, explicit package-local execution, and the sole lockfile. Pre-commit hooks stay fast and focused; pre-push covers the wider validation required before CI. Never bypass a failing gate, disable a test, or skip hooks merely to make a change pass. A badge, prior run, generated file, or agent self-report is not evidence for the current revision; pending, skipped, cancelled, timed-out, blocked, or unobserved work is not green.

Before merging, read the effective repository and organization review/check rules, satisfy each required review with qualifying independent GitHub approval of the final head, and verify successful current required checks. An owner/admin exemption, successful merge API response, local agent review, or earlier-head approval is not a substitute for required approval. Unknown review eligibility or unreadable rules block acceptance; do not change protections or grant access merely to manufacture a passing gate. The owner may deliberately invite a genuinely trusted independent reviewer with the minimum role counted by the effective rules for substantive review; that invitation is not approval, must not be automated or inferred, and does not authorize any merge. On repositories where GitHub bundles review eligibility with broader write capabilities, the owner must assess that access explicitly, keep protections active, and retain or remove the role only through a deliberate lifecycle decision after every receipt that depends on current eligibility has been captured. If a security bootstrap lands without the required review, keep its new hosted consumers inactive until an independently approved corrective PR explicitly reviews the entire helper/generator/generated-artifact trust boundary at its final head, not only the follow-up diff.
