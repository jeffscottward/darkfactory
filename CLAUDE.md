...
When effective rules require independent approval, it must qualify for the final head. An owner/admin exemption, successful merge API response, local agent review, or earlier-head approval is not a substitute for required approval; unknown review eligibility blocks acceptance. The owner may deliberately invite a genuinely trusted independent reviewer with the minimum role counted by the effective rules for substantive review; that invitation is not approval, must not be automated or inferred, and does not authorize any merge. On repositories where GitHub bundles review eligibility with broader write capabilities, the owner must assess that access explicitly, keep protections active, and retain or remove the role only through a deliberate lifecycle decision after every receipt that depends on current eligibility has been captured.
